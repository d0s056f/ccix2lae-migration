...
<div class="page_body">
	...
	<h3 align="center">
		...
		&nbsp;${(dir_name).split(': ')[1]} - Committed Apps
	</h3>
	<hr>
	<br>
				...
				<div class="panel panel-default">
					<div class="panel-body">
						<table id="tableBranchData"
							...
							<c:forEach items="${dircomm_table}" var="dircomm"><tr>
								<td>${dircomm.app_id}</td>
								<td>
								<c:choose>
											<c:when test='${dircomm.app_name == null}'>
       N/A
    </c:when>
											<c:otherwise>
        ${dircomm.app_name}
								...
								<td>${dircomm.func_grp}</td>
								<td style="text-align: center;">${dircomm.svp_name.split(': ')[1]}</td>
								<td style="text-align: center;">
								<c:choose>
											<c:when test='${dircomm.eman_priority == null}'>
       N/A
    </c:when>
											<c:otherwise>
        ${dircomm.eman_priority}
    </c:otherwise>
										</c:choose></td>
								<td style="text-align: center;">${dircomm.mig_date}</td>
							</tr></c:forEach>
