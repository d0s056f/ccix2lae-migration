...
<div class="page_body">
	...
	<h3 align="center">
		...
		&nbsp;${(dir_name).split(': ')[1]} - Completed Apps
	</h3>
	<hr>
	<br>
				...
				<div class="panel panel-default">
					<div class="panel-body">
						<table id="tableBranchData"
							...
							<c:forEach items="${dircomp_table}" var="dircomp"><tr>
								<td>${dircomp.app_id}</td>
								<td>
								<c:choose>
											<c:when test='${dircomp.app_name == null}'>
       N/A
    </c:when>
											<c:otherwise>
        ${dircomp.app_name}
								...
								<td>${dircomp.func_grp}</td>
								<td style="text-align: center;">${dircomp.svp_name.split(': ')[1]}</td>
								<td style="text-align: center;">
								<c:choose>
											<c:when test='${dircomp.eman_priority == null}'>
       N/A
    </c:when>
											<c:otherwise>
        ${dircomp.eman_priority}
    </c:otherwise>
										</c:choose></td>
								<td style="text-align: center;">${dircomp.mig_date}</td>
							</tr></c:forEach>
