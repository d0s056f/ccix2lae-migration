...
<div class="page_body">
	...
	<h3 align="center">
		...
		&nbsp;${(dir_name).split(': ')[1]} - In Progress Apps
	</h3>
	<hr>
	<br>
						...
						<table id="tableBranchData"
							...
							<thead>
								<tr style="color: #346581;">
									...
									<th style="text-align: center;">Senior Staff</th>
									<th style="text-align: center;">EMAN Priority</th>
									<th style="text-align: center;">Migration Date</th>
								</tr>
							</thead>
							<c:forEach items="${dirprog_table}" var="dirprog"><tr>
								<td>${dirprog.app_id}</td>
								<td>
								<c:choose>
											<c:when test='${dirprog.app_name == null}'>
       N/A
    </c:when>
											<c:otherwise>
        ${dirprog.app_name}
								...
								<td>${dirprog.func_grp}</td>
								<td style="text-align: center;">${dirprog.svp_name.split(': ')[1]}</td>
								<td style="text-align: center;">
								<c:choose>
											<c:when test='${dirprog.eman_priority == null}'>
       N/A
    </c:when>
											<c:otherwise>
        ${dirprog.eman_priority}
    </c:otherwise>
										</c:choose></td>
								<td style="text-align: center;">${dirprog.mig_date}</td>
							</tr></c:forEach>
